...
namespace BookManage.Utils
{
    public class SqlFactoryUtil
    {
        ...
        /// <returns></returns>
        public List<T> Query<T>(string whereClause, System.Data.IDbDataParameter[] parms = null) where T : class, new()
        {
            ...
            if (tblAttr != null)
            {
                tableName = (tblAttr as TableNameAttribute).Name;
            }
            StringBuilder s_sql = new StringBuilder();
            s_sql.Append("SELECT * FROM " + tableName);
            if (!string.IsNullOrEmpty(whereClause))
            {
                s_sql.Append(" WHERE " + whereClause);
            }
            DataTable table = QueryDataSet(s_sql.ToString(), parms).Tables[0];
